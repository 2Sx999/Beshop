...
<HEAD>
    ...
    <script type="text/javascript">
        ...
        function findOrderInfoByOid(oid) {
            ...
            //ajax异步访问数据
            $.post(
                "${pageContext.request.contextPath }/admin.php?method=findOrderItemByOid",
                {"oid": oid},
                function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        content += "<tr style='text-align: center;'>" +
                            "<td>" +
                            "<img src='${pageContext.request.contextPath }/" + data[i].product.pimage + "' width='60' height='60'>" +
                            "</td>" +
...
<div class="pagenation">
    <c:if test="${pageBean.currentPage-1>0}">
        <a href="${ pageContext.request.contextPath }/admin.php?method=getOrdersByPagination&currentPage=${pageBean.currentPage-1}">上一页</a>
    </c:if>
    <a href="javascript:void(0);">${pageBean.currentPage}</a>
    <c:if test="${pageBean.currentPage+1<=pageBean.totalPage}">
        <a href="${ pageContext.request.contextPath }/admin.php?method=getOrdersByPagination&currentPage=${pageBean.currentPage+1}">下一页</a>
    </c:if>
</div>
